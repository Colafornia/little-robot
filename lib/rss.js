        ...
        parser.parseURL(source.url, function(err, parsed) {
            ...
            if (parsed && parsed.feed && parsed.feed.entries.length &&
                updatedInLastWeek(parsed.feed.entries).length) {
                    log.info(`${source.title} 本周有新文章 ${updatedInLastWeek(parsed.feed.entries).length} 篇`)
                ...
                log.info(`${source.title} 本周有新文章0篇`)
            }
            resolve('success');
        })
    })
...
const updatedInLastWeek = function (entries) {
    ...
    while (list[0] && list[0].pubDate && moment(new Date (list[0].pubDate)).startOf('week').isSame(moment().startOf('week'), 'week')) {
        result.push(list[0]);
        list.shift();
    }
    return result;
